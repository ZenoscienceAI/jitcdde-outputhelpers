...
typedef struct
{
	...
	double helpers[{{number_of_helpers}}];
	{% endif %}
	...
	double dY[{{n}}])
{
	...
	# include "helpers.c"
	/* Store the just-computed helpers for later retrieval */
	memcpy(self->helpers, f_helper, sizeof(f_helper));
	{% endif %}
	# include "f.c"
}
...
{% if number_of_helpers>0 %}
/* ------------------------------------------------------------------
 *  Return a read-only NumPy view of the helper values that belong to
 *  the *current* integration point (self->current).
 * ------------------------------------------------------------------ */
static PyObject *
...
static PyMethodDef dde_integrator_methods[] = {
	...
	{% if number_of_helpers>0 %}
	{"get_helpers", (PyCFunction) get_helpers, METH_NOARGS,
	 "Return the helper values computed at the current integration point."},
	{% endif %}
